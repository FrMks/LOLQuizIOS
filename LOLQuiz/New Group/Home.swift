...
struct Home: View {
            ...
            ForEach($rows, id: \.self) { $row in
                HStack(spacing: 10) {
                    ForEach($row) { $item in
                        ...
                        Text(item.value)
                            .font(.system(size: item.forntSize))
                            .padding(.vertical, 5)
                            ...
                            .background {
                                // If Item us Dropped into Correct Place
                                RoundedRectangle(cornerRadius: 6, style: .continuous)
                                    .stroke(.gray)
                                    .opacity(item.isShowing ? 1 : 0)
                            }
                                ...
                                if let first = providers.first {
                                    let _ = first.loadObject(ofClass: URL.self) { value, error in
                                        ...
                                        if item.id == "\(url)" {
                                            ...
                                            withAnimation {
                                                item.isShowing = true
                                                updationShuffledArray(charecter: item)
                                                self.progress = progress
                                            }
                                        }
                                        else {
                                           // Animation when wrong text Dropped
                                            animateView()
                                        }
                                    }
                                }
                ...
                HStack(spacing: 10) {
                    ForEach(row) { item in
                        ...
                        //MARK: Adding Drag Operation
                            .onDrag {
                                // Return ID to find which Item is Moving
    ...
    func textSize(character: Character) -> CGFloat {
        let font = UIFont.systemFont(ofSize: character.forntSize)
    ...
    func animateView() {
        withAnimation(.interactiveSpring(response: 0.3, dampingFraction: 0.32, blendDuration: 0.2)) {
